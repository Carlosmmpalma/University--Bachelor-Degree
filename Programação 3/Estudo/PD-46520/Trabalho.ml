...
let process_uppercase s i =
  ...
  if p = 'R' then begin (*ROOK-TORRE*)
    let come = s.[1] in
    if come <> 'x' then begin (*Jogada em que não se come peça*)
      let x = Char.code s.[2] -48 in
      let y = letter_to_number s.[1] in
      print_int x;
      print_int y;
      if i mod 2 = 0 then begin
        print_endline (Printf.sprintf "Jogador1: %s" s);
        if board.(x).(y) = "R" then begin
          board.(x).(y) <- "R";
          (* code to update the position of the rook goes here *)
          print_endline "possible"
        end else begin
    ...
    end else begin (*Jogada em que se come peça*)
      print_endline "come"
      (* code to process a capture goes here *)
    end
  end else begin
    print_endline "outro"
  end
...
let ler_jogadas (file : string) : string list =
  let canal = open_in file in
  let rec loop acc =
    try
